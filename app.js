...
router.post(apiUlr + 'adduser', (req, res, next) => {
    ...
    const user = req.body;
    console.log(user);
    if (!user.name || !user.lastname || !user.email || !user.password || !user.carrier || !user.semester) {
        return res.json({succes: false, error: 'Por favor envie parametros necesarios'});
    }
...
router.post(apiUlr + 'login', function (req, res, next) {
    ...
    adminDb.collection('users').find({email: user.email, password: user.password})
        .toArray(function (err, item) {
            if (err) {
                return res.json({success: false, err});
            }
            res.json({success: true, user: item[0]});
        });
});
